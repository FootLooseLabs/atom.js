...
AtomCmpInterface.prototype._bindConnections = function (argument) {
  ...
  //   console.debug(`DEBUG: Atom.Interface${this.name} Binding Connection:<${key}>`);

  //   let connTargetInterfaceName = this._getConnTargetInterfaceName(_interfaceConnectionsConfig[key]);
...
/**
 * Generate a REQ/REP request handler from an existing lexeme-based component function
 * This allows existing PUB/SUB-style component functions to work with REQ/REP pattern
 * @private
 * @param {string} lexemeName - Name of the lexeme/operation
 * @param {class} lexemeClass - Lexeme class definition
 * @returns {function} - Auto-generated async handler function
 */
AtomCmpInterface.prototype._generateRequestHandler = function (
  lexemeName,
  lexemeClass,
) {
  ...
  return async function (data, context) {
    // context = { correlationId, sender, timestamp }

    // Check if component function exists
    if (!component[lexemeName]) {
      throw new Error(
        `Component function '${lexemeName}' not found for auto-generated handler`,
      );
    }

    // Reconstruct message format expected by existing component functions
    // This maintains backward compatibility with PUB/SUB pattern
    const reconstructedMessage = {
      ...data,
      // Add sender in PUB/SUB format for backward compatibility
      // Note: REQ/REP sender won't have :::PublishToWsClients routing info,
      // but component functions should handle this gracefully
      sender: context.sender,
    };

    // Inflect the message using the lexeme (same as PUB/SUB path)
    let inflection;
    try {
      inflection = lexemeClass.inflect(
        JSON.stringify(reconstructedMessage),
        [],
      );
      if (!inflection) {
        throw new Error(
          `Inflection failed for '${lexemeName}' - lexeme returned false`,
        );
      }
    } catch (err) {
      throw new Error(
        `Failed to inflect message for '${lexemeName}': ${err.message}`,
      );
    }

    // Call the existing component function (same as PUB/SUB path)
    // Component functions are expected to be async and return { result, message, error }
    let result;
    try {
      result = await component[lexemeName](inflection.get(), []);

      // Handle response format (backward compatible with existing component functions)
      if (!result) {
        // Some component functions may return nothing for fire-and-forget operations
        return { message: "Operation completed" };
      }

      // Check for errors in component response
      if (result.error) {
        throw new Error(
      ...
      // Return the result portion for Atom.Request response
      // Component functions return { result, message, error }
      // We extract just the result for REQ/REP response
      return result.result !== undefined ? result.result : result;
    } catch (err) {
      // Re-throw with context for better debugging
      throw new Error(
        `Component '${lexemeName}' execution error: ${err.message}`,
      );
    }
  };
};
...
AtomCmpInterface.prototype._setupRequestIntegration = function () {
  try {
    ...
    // Define lexemes that should NOT be auto-generated as request handlers
    const BASE_LEXEMES = ["GetIntro", "Update", "Response", "Advertisement"];

    // Special operations that don't follow request-response pattern
    // These are typically fire-and-forget or gateway-specific operations
    const SPECIAL_OPERATIONS = [
      "PublishToWsClients", // WebSocket gateway - async message delivery
      "ListWsClients", // Gateway CRUD operations
      "RemoveWsClient",
      "AddOrUpdateWsClient",
    ];

    const skipLexemes = new Set([...BASE_LEXEMES, ...SPECIAL_OPERATIONS]);

    // Track auto-generated handlers for logging
    const autoGeneratedHandlers = [];

    // 1. Register explicit request handlers from config (takes precedence)
    if (this.config.requestHandlers) {
      for (const [operation, handler] of Object.entries(
        ...
        if (typeof handler === "function") {
          ...
          console.debug(
            chalk.green(
              `AtomRequest: Registered explicit handler for '${operation}'`,
            ),
          );
        }
      }
    ...
    // 2. AUTO-GENERATE handlers from lexicon for backward compatibility
    // This allows existing PUB/SUB component functions to work with REQ/REP
    for (const [lexemeName, lexemeClass] of Object.entries(
      this.config.lexicon,
    )) {
      // Skip base framework lexemes
      if (skipLexemes.has(lexemeName)) {
        console.debug(
          `AtomRequest: Skipping auto-generation for base/special lexeme '${lexemeName}'`,
        );
        continue;
      }

      // Skip if explicitly defined in requestHandlers (explicit takes precedence)
      if (
        this.config.requestHandlers &&
        this.config.requestHandlers[lexemeName]
      ) {
        console.debug(
          `AtomRequest: Skipping auto-generation for '${lexemeName}' - explicit handler exists`,
        );
        continue;
      }

      // Check if component function exists before generating handler
      if (!component[lexemeName]) {
        console.debug(
          `AtomRequest: Skipping auto-generation for '${lexemeName}' - no component function`,
        );
        continue;
      }

      // Auto-generate handler from lexeme definition
      try {
        const autoHandler = this._generateRequestHandler(
          lexemeName,
          lexemeClass,
        );
        AtomRequest.handle(lexemeName, autoHandler);
        autoGeneratedHandlers.push(lexemeName);
        console.debug(
          chalk.blue(
            `AtomRequest: Auto-generated REQ/REP handler for '${lexemeName}' from lexicon`,
          ),
        );
      } catch (err) {
        console.warn(
          chalk.yellow(
            `AtomRequest: Failed to auto-generate handler for '${lexemeName}': ${err.message}`,
          ),
        );
      }
    }
    ...
    console.log(
      chalk.green(
        `AtomRequest: Interface setup completed for ${this.name}`,
      ),
    );
    if (autoGeneratedHandlers.length > 0) {
      console.log(
        chalk.blue(
          `AtomRequest: Auto-generated ${autoGeneratedHandlers.length} REQ/REP handlers: [${autoGeneratedHandlers.join(", ")}]`,
        ),
      );
    }
