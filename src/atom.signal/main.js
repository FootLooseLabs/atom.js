...
AtomSignal.prototype._connect = function () {
  try {
    ...
    if (this.isSubscriber == true) {
      console.log(
        `Info: Signal ready to receive wavelets from - ${this.address}`,
      );
    } else {
...
AtomSignal.prototype.sendWavelet = function (channel, payload, paramList) {
  ...
  try {
    if (typeof payload != "string") {
      //case of cli - the json inputs are already string
    ...
    console.log(
      `Info: ${this.getLabel()} published wavelet = `,
      `${channel}:::${payload}`,
    );
  } catch (e) {
    throw `Error: ${e.message}`;
...
AtomSignal.constructFromSpec = async (signalSpec) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      var [interfaceAddress, channel] =
        signalSpec.get().signalType == "subscriber"
          ? signalSpec.get().targetInterfaceLabel.split("|||")
          : signalSpec.get().targetInterfaceLabel.split(":::");
    } catch (e) {
      reject(e);
    }
    ...
    console.debug(
      `Atom.Signal (type=${signalSpec.get().signalType}) being setup with - `,
      `${interfaceAddressWithPrefix}:::${channel}`,
    );

    try {
      var interfaceSpec = await Nucleus.getInterfaceIfActive(
        interfaceAddressWithPrefix,
      );
    } catch (err) {
      status.update({
        error: err,
    ...
    if (!interfaceSpec) {
      //case when atom.nucleus is not running
    ...
    try {
      signal = new AtomSignal({
        ...
        isSubscriber:
          signalSpec.get().signalType == "subscriber" ? true : false,
        channel: channel,
      });
      status.update({
        error: null,
...
AtomSignal.publishToInterface = async (interfaceLabel, message, paramList) => {
  ...
  try {
    var establishedSignalStatus =
      await AtomSignal.constructFromSpec(signalSpec);
  } catch (e) {
    throw e;
  }

  console.debug(
    "DEBUG: AtomSignal: Successfully Established - \n",
    establishedSignalStatus.signal.getLabel(),
  );
...
AtomSignal.subscribeToInterface = async (interfaceLabel) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    signal.sock.on("message", async (_topicName, message) => {
      ...
      console.log(
        chalk.yellow(`${signal.getLabel()}:::${epoch} -
      ...
      if (_topicName == signal.channel) {
        ...
        console.debug(
          chalk.blue(
            `EVENT:::${signal.getLabel()} - ${_inflection.stringify()}`,
          ),
        );
...
AtomSignal.unsubscribeFromInterface = async (signal) => {
  return new Promise(async (resolve, reject) => {
    var status = LEXICON.SignalStatus.inflect({
      op: signal ? signal.getLabel() : "unknown-signal",
      error: false,
      message: "",
      statusCode: 0,
    });
    ...
    if (!signal) {
      status.update({
        error: "Error: No signal provided",
        message: "AtomSignal: Cannot unsubscribe - no signal provided",
        statusCode: -1,
      });
      console.error(status.get().message);
      reject(status.get());
    ...
    if (signal.signalType !== "subscriber") {
      status.update({
        error: "Error: Signal is not a subscriber",
        message: `AtomSignal: Cannot unsubscribe - ${signal.getLabel()} is not a subscriber`,
        statusCode: -1,
      });
      console.error(status.get().message);
      reject(status.get());
    ...
    if (signal.ended) {
      status.update({
        error: false,
        message: `AtomSignal: ${signal.getLabel()} already unsubscribed`,
        statusCode: 1,
      });
      console.debug(status.get().message);
      resolve(status.get());
    ...
    try {
      // Unsubscribe from the ZMQ channel
      if (signal.sock && signal.channel) {
        signal.sock.unsubscribe(signal.channel);
        console.debug(
          `AtomSignal: Unsubscribed from channel ${signal.channel}`,
        );
      }

      // Remove all event listeners
      if (signal.eventEmitter) {
        signal.eventEmitter.removeAllListeners();
        console.debug(
          `AtomSignal: Removed all event listeners for ${signal.getLabel()}`,
        );
      }

      // Destroy the signal (closes socket)
      signal.destroy();

      status.update({
        error: false,
        message: `AtomSignal: Successfully unsubscribed ${signal.getLabel()}`,
        statusCode: 2,
      });
      ...
      status.update({
        error: e.message,
        message: `AtomSignal: Error unsubscribing ${signal.getLabel()} - ${e.message}`,
        statusCode: -1,
      });
      console.error(status.get().message);
      reject(status.get());
